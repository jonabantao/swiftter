...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    ...
    func postTweet(tweet: String, success: @escaping () -> (), failure: @escaping (Error) -> ()) {
        let postTweetURL = "https://api.twitter.com/1.1/statuses/update.json"
        
        TwitterAPICaller.client?.post(postTweetURL, parameters: [ "status": tweet ], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
        })
    ...
    func unfavoriteTweet(tweetId: Int, success: @escaping () -> (), failure: @escaping (Error) -> ()) {
        let favoriteURL = "https://api.twitter.com/1.1/favorites/destroy.json"
    ...
    func retweet(tweetId: Int, success: @escaping () -> (), failure: @escaping (Error) -> ()) {
        let favoriteURL = "https://api.twitter.com/1.1/statuses/retweet/:id.json"
    ...
    func unretweet(tweetId: Int, success: @escaping () -> (), failure: @escaping (Error) -> ()) {
        let favoriteURL = "https://api.twitter.com/1.1/statuses/unretweet/:id.json"
    ...
    func getCurrentUserData(success: @escaping (NSDictionary) -> (), failure: @escaping (Error) -> ()) {
        let credentialsURL = "https://api.twitter.com/1.1/account/verify_credentials.json"
        
        TwitterAPICaller.client?.get(credentialsURL, parameters: nil, progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success(response as! NSDictionary)
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
        })
